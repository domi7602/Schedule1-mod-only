...
Analyses based on ~1855 C# classes from the `Il2CppScheduleOne` namespace.

## Code References

Vanilla decompiled classes are located in:
`GameReferences/decompiled/Assembly-CSharp/Il2CppScheduleOne/`

## Workflow Guidelines for Agents

- Locate target system using the table above and read the respective markdown file.
- Locate core classes directly in `GameReferences/decompiled/Assembly-CSharp/Il2CppScheduleOne/<System>/`.
- For S1API wrappers, check `ThirdParty/S1API/S1API/`.
- For IL2CPP Interop rules & safety patterns, see `Skills/schedule1-modding/SKILL.md` and `Skills/schedule1-troubleshooting/SKILL.md`.
