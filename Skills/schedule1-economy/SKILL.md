...
**Truth:** Cash is *inventory*, not a float. `BankApp` `BankService.cs` calculates holdable capacity via `GetMaxHoldableCashCapacity(maxPerSlot)`. Forgetting the slot check = **cash loss** (withdraw destroys bills).
...
└─ Customer / laundering / business unlock?
   → Skills/schedule1-game-systems/references/23-Economy-Money.md + 11-Business-Laundering.md + 19-Customer-Budget.md
```
