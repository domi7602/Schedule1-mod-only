...
```pwsh
# List S1API source classes
rg -l "public (class|sealed class|abstract class|static class) " "ThirdParty/S1API/S1API/" --type cs

# Find a specific namespace
Get-ChildItem "ThirdParty/S1API/S1API/Quests"

# Find a method signature
rg -n "public static.*CreateQuest" "ThirdParty/S1API/S1API/Quests"

# Verify deployed S1API assembly against the running game
& (Get-Command ilspycmd -ErrorAction Stop).Source -t "S1API.Money.Money" "$env:SCHEDULE1_PATH\Mods\S1API.Il2Cpp.MelonLoader.dll"
