...
public static class BountyCallScheduler
{
    ...
    /// <summary>
    /// Caller picks a reward from a stable distribution per style.
    ///   cold       =  8 000 – 25 000
    ///   threatening = 12 000 – 30 000
    ///   desperate  = 18 000 – 45 000
    /// (Higher reward for high-pressure NPCs.)
    /// </summary>
    private static float RewardFor(string style)
    {
        switch (style)
        {
            case BountyDialogTemplates.StyleThreatening:
                return 12_000 + Rng.Next(0, 18_001);
            case BountyDialogTemplates.StyleDesperate:
                return 18_000 + Rng.Next(0, 27_001);
            default:
                return 8_000 + Rng.Next(0, 17_001);
        }
    }
