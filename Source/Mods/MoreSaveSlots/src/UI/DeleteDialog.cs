...
public static class DeleteDialog
{
    ...
    private static TextMeshProUGUI? _warningTmp;
    ...
    public static void OpenForHoveredOrSelected()
    {
        ...
        // No populated save on page — nothing to delete
        MelonLogger.Warning("[MoreSaveSlots] No save to delete on current page.");
    }
    ...
    public static void Open(int slotIndex)
    {
        ...
        if (LoadManager.SaveGames != null && slotIndex >= 0 && slotIndex < LoadManager.SaveGames.Length)
            info = LoadManager.SaveGames[slotIndex];

        if (info == null)
        {
            MelonLogger.Warning($"[MoreSaveSlots] Cannot delete: Slot {slotIndex + 1} is empty.");
            return;
        }

        if (RenameDialog.IsOpen) RenameDialog.Close();
        EnsureDialogCreated();
    ...
    public static void SelectSlot(int slotIndex)
    {
        ...
        if (_titleTmp != null)
            _titleTmp.text = $"DELETE SAVE — SLOT {slotIndex + 1}";

        if (_subTmp != null)
        {
            string org = saveInfo?.OrganisationName ?? "(Empty)";
            string date = saveInfo != null ? saveInfo.DateLastPlayed.ToString("yyyy-MM-dd HH:mm") : "";
    ...
    private static void RefreshSlotSelectorButtons()
    {
        ...
        Color activeCol = new Color(0.55f, 0.18f, 0.18f, 1f);
    ...
    public static void Confirm()
    {
        ...
        Close();
        SaveDeleteService.DeleteSave(slot);
    }
    ...
    private static void EnsureDialogCreated()
    {
        ...
        winRt.sizeDelta = new Vector2(500f, 300f);
        ...
        _titleTmp = UIHelper.CreateTextMeshPro(winObj.transform, "Title", "DELETE SAVE", 16f, FontStyles.Bold, TextAlignmentOptions.Center, new Color(0.95f, 0.45f, 0.45f, 1f));
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            Button slotBtn = UIHelper.CreateButton(
                ...
                () =>
                {
                    ...
                    SaveInfo? si = null;
                    if (LoadManager.SaveGames != null && actual >= 0 && actual < LoadManager.SaveGames.Length)
                        si = LoadManager.SaveGames[actual];
                    if (si != null)
                        SelectSlot(actual);
                },
        ...
        _subTmp = UIHelper.CreateTextMeshPro(winObj.transform, "Subtitle", "Slot info", 12f, FontStyles.Bold, TextAlignmentOptions.Center, new Color(0.95f, 0.95f, 0.98f, 1f));
        ...
        _warningTmp = UIHelper.CreateTextMeshPro(winObj.transform, "Warning", "This will <b><color=#FF5555>permanently delete</color></b> the save file.\nThis cannot be undone.", 11f, FontStyles.Normal, TextAlignmentOptions.Center, new Color(0.85f, 0.65f, 0.65f, 1f));
        RectTransform warnRt = _warningTmp.GetComponent<RectTransform>();
        warnRt.anchorMin = new Vector2(0f, 1f);
        warnRt.anchorMax = new Vector2(1f, 1f);
        warnRt.pivot = new Vector2(0.5f, 1f);
        warnRt.sizeDelta = new Vector2(440f, 40f);
        warnRt.anchoredPosition = new Vector2(0f, -115f);
        ...
        UIHelper.CreateButton(
            ...
            "DELETE",
            140f, 36f,
            new Color(0.55f, 0.14f, 0.14f, 0.95f),
