...
public static class ReflectionHelper
{
    ...
    /// <summary>
    /// Finds all active/scene GameObjects safely without scanning thousands of internal assets.
    /// </summary>
    /// <returns>List of GameObjects in active scenes.</returns>
        ...
        try
        {
            ...
            if (activeScene.IsValid() && activeScene.isLoaded)
            {
                ...
                foreach (var root in roots)
                {
                    if (root != null)
                    {
                        AddHierarchy(root, results, maxDepth: 10);
                    }
                }
            }
        }
    ...
    private static void AddHierarchy(GameObject obj, List<GameObject> results, int maxDepth)
    {
        if (obj == null || results.Count >= 2000 || maxDepth <= 0) return;
        results.Add(obj);
        for (int i = 0; i < obj.transform.childCount; i++)
        {
            var child = obj.transform.GetChild(i);
            ...
            {
                AddHierarchy(child.gameObject, results, maxDepth - 1);
            }
        }
    }
